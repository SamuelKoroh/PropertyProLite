...
*/
  async signUp({ body, file }, res) {
    const errors = Joi.validate(body, signupSchema);
    if (errors.error) return res.status(400).json({ status: 'error', error: errors.error.details[0].message });
    ...
    try {
      ...
      Users.push(user);
      user.token = await jwt.sign(_.pick(user, ['id']), jwtSecret, { expiresIn: 3600 });
...
*/
  async signIn({ body }, res) {
    try {
      ...
      if (!user) return res.status(400).json({ status: 'error', error: 'Invalid username and password' });

      const validPassword = await bcrypt.compare(body.password, user.password);
      if (!validPassword) return res.status(400).json({ status: 'error', error: 'Invalid username and password' });
